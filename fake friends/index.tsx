...
const userContextPatch: NavContextMenuPatchCallback = (children, props) => {
    ...
    try {
        ...
        if (!state) {
            items = [
                <Menu.MenuItem key="ff-friend" id="ff-friend" label="Agregar como amigo falso" action={() => doFakeFriend(userId)} />,
                <Menu.MenuItem key="ff-request" id="ff-request" label="Crear solicitud de amistad falsa" action={() => doFakeFriendRequest(userId)} />,
            ];
        } else if (state === "pending") {
            items = [
                <Menu.MenuItem key="ff-cancel" id="ff-cancel" label="Retirar solicitud de amistad falsa" color="danger"
                    action={async () => {
            ...
            items = [
                <Menu.MenuItem key="ff-remove" id="ff-remove" label="Retirar como amigo falso" color="danger"
                    action={async () => {
                        fakeState.delete(userId);
                        await persistState();
                        FluxDispatcher.dispatch({ type: "RELATIONSHIP_REMOVE", relationship: { id: userId } });
...
const guildContextPatch: NavContextMenuPatchCallback = (children, props) => {
    ...
    try {
        ...
        const items = [
            <Menu.MenuItem key="ff-g-flood" id="ff-g-flood" label="Crear solicitud de amistad falsa"
        ...
        if (fakeCount > 0) {
            items.push(
                <Menu.MenuItem
                    key="ff-g-remove"
                    id="ff-g-remove"
                    label={`Retirar solicitudes de amistad falsas (${fakeCount})`}
                    color="danger"
...
export default definePlugin({
    ...
    description: "Simula amigos / solicitudes de amistad falsas",
